...
$conn = $connection->query ("select MAC,ESSID,TIME,TRILAT,TRILONG,LASTSEEN from SCANNER  where ESSID in (select ESSID from BEACON  where TIME < DATE_ADD('".$time."', INTERVAL 30 SECOND) AND TIME > DATE_ADD('".$time."', INTERVAL -30 SECOND))");
while ($row = $conn->fetch()) {
	$resultNum.=1;
	$scannerResults.='<tr><td>'.$row['MAC'].'</td><td>'.str_replace("+"," ",$row['ESSID']).'</td>
